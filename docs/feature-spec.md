...
- Windows uses .NET 8, WPF, a system-tray icon, Windows startup integration, and native process-launch APIs.
...
Codex Duo itself does not implement an account-usage network client. Network activity needed by refresh, login, or switching is delegated to the platform's configured command-line helper.
...
Rate-limit windows are rendered from the source data without inferring missing values. The UI uses familiar labels for `300` minutes (5H), `10080` minutes (WEEK), and the Free `43200`-minute window (MONTH); other reported windows receive a duration label.
...
| Reported usage-window display | Implemented | Implemented | macOS includes Free monthly windows; missing windows are omitted. |
